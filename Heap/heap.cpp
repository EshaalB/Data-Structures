...
//    int rightChild = 2 * index + 2;
//
//    if (leftChild < size && heap[index]<heap[smallest]) { smallest = leftChild; }
//    if (rightChild < size && heap[index]<heap[smallest]) { smallest = rightChild; }
...
//    return maxHeap;
//}
//bool isHeap(vector<int> minHeap) {
...
//        int left = 2 * i + 1;
//        int right = 2 * i + 2;
//        if (left <index &&minHeap[i] > minHeap[left]) return false;
//        if (left <index &&  minHeap[i] > minHeap[right]) return false;
//
//    }
//    return true;
//}
////void BTtoMinHeap(Node* root, int index, vector<int>& arr) {
////    if (root == nullptr || index >= arr.size()) return;
////
////    root->data = arr[index];
////
////    // Recur for the left and right subtrees
////    BTtoMinHeap(root->left, 2 * index + 1, arr);
////    BTtoMinHeap(root->right, 2 * index + 2, arr);
////}
////
...
////    if (n <= 0) return -1;  // Empty heap
////
////    int minElement = heap[0];  // Root of the heap
////
////    // Replace root with last element and reduce heap size
////    heap[0] = heap[n - 1];
////    n--;
